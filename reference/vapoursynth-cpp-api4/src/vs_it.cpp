...
IT::IT(VSVideoInfo * vi, VSNode * node, int _fps, int _threshold, int _pthreshold, const VSAPI * vsapi) :
	m_iFPS(_fps),
...
void IT::GetFramePre(IScriptEnvironment * env, int n) {
	// API4 port: widen the request range so getFrameFilter (the only legal
	// frame access from inside a filter's getFrame in API4) can satisfy all
	// reads. The original code only requests [base, base+5] for fps=24 and
	// {n} for fps=30, then relies on the API3 sync vsapi->getFrame to pull
	// neighbours from the cache — that's no longer available under API4 and
	// would deadlock/segfault.
	if (m_iFPS == 24) {
		int base = n + n / 4;
		base = (base / 5) * 5;
		for (int i = base - 2; i <= base + 6; ++i)
			env->vsapi->requestFrameFilter(clipFrame(i), node, env->frameCtx);
	}
	else {
...
const VSFrame* IT::GetFrame(IScriptEnvironment * env, int n) {
	++m_iCounter;
	env->m_iRealFrame = n;
	...
	VSFrame * dst = env->NewVideoFrame(vi);
...
const VSFrame* IT::MakeOutput(IScriptEnvironment * env, VSFrame * dst, int n) {
	env->m_iCurrentFrame = n;
