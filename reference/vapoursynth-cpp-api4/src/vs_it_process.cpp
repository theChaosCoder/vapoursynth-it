...
void IT::DeintOneField_YV12(IScriptEnvironment * env, VSFrame * dst, int n) {
...
bool IT::DrawPrevFrame(IScriptEnvironment * env, VSFrame * dst, int n) {
	bool bResult = false;
...
void IT::CopyCPNField(IScriptEnvironment * env, VSFrame * dst, int n) {
	const VSFrame * srcC = env->GetFrame(clipFrame(n));
	const VSFrame * srcR;
	switch (toupper(env->m_iUseFrame)) {
		default:
