...
void IT::EvalIV_YV12(IScriptEnvironment * env, int n, const VSFrame * ref, long & counter, long & counterp) {
	const VSFrame * srcC = env->GetFrame(clipFrame(n));
...
void IT::MakeDEmap_YV12(IScriptEnvironment * env, const VSFrame * ref, int offset) {
...
void IT::MakeMotionMap_YV12(IScriptEnvironment * env, int n, bool flag) {
	...
	const VSFrame * srcP = env->GetFrame(clipFrame(n - 1));
	const VSFrame * srcC = env->GetFrame(n);
...
void IT::MakeMotionMap2Max_YV12(IScriptEnvironment * env, int n) {
	...
	const VSFrame * srcP = env->GetFrame(n - 1);
	const VSFrame * srcC = env->GetFrame(n);
	const VSFrame * srcN = env->GetFrame(n + 1);
