...
public:
    ...
    const VSFrame* VS_CC GetFrame(IScriptEnvironment* env, int n);
    const VSFrame* VS_CC MakeOutput(IScriptEnvironment* env, VSFrame* dst, int n);
    bool VS_CC CheckSceneChange(IScriptEnvironment* env, int n);
    void VS_CC GetFrameSub(IScriptEnvironment* env, int n);

    void VS_CC EvalIV_YV12(IScriptEnvironment* env, int n, const VSFrame * ref, long &counter, long &counterp);
    void VS_CC MakeDEmap_YV12(IScriptEnvironment* env, const VSFrame * ref, int offset);
    ...
    void VS_CC CopyCPNField(IScriptEnvironment* env, VSFrame * dst, int n);
    void VS_CC DeintOneField_YV12(IScriptEnvironment* env, VSFrame * dst, int n);
    ...
    bool VS_CC DrawPrevFrame(IScriptEnvironment* env, VSFrame * dst, int n);
};
